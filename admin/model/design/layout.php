	...
	public function addLayout(array $data): int {
		...
		if (isset($data['layout_module'])) {
			foreach ($data['layout_module'] as $layout_module) {
				// echo '<pre>';print_r($layout_module);exit;
	...
	public function editLayout(int $layout_id, array $data): void {
		...
		if (isset($data['layout_route'])) {
			foreach ($data['layout_route'] as $layout_route) {
				...
				// 	print_r(("INSERT INTO `" . DB_PREFIX . "layout_route` SET `layout_id` = '" . (int)$layout_id . "', `store_id` = '" . (int)$layout_route['store_id'] . "', `route` = '" . $this->db->escape($layout_route['route']) . "'"));exit;
				// }
			}
		}
