...
;(function ($) {
	$(document).ready(function(){
		if ($('.jetimpex-product-slider').length) {
			var slider           = {};
			window.productSlider = [];
			$('.jetimpex-product-slider').each(function(){
				$this = $(this);
				...
				window.productSlider.push(slider);
			});
		...
		$(document).ready(function () {
			o.each(function () {
				...
				.each(function () {
					var $this = $(this),
					url;
					pagTitles.push($this.data('pag-title'));
				...
				.swiper({
					...
					paginationBulletRender: pag.length ? pag.attr("data-index-bullet") === "true" ? function (index, className) {
						return '<span class="' + className + '">' + '<span class="index">' + (((index + 1) < 10) ? ("0" + (index + 1)) : (index + 1)) + '</span>' + '</span>';
					} : null : null,
...
;(function ($) {
	$(document).ready(function(){
		var mCar = $('[data-mobile-carousel]');
		if (mCar.length) {
			var mobileWidth = $(window).width() > 767 ? false : true,
			created = false;
			$(window).on('load resize', function(){
				mobileWidth = $(window).width() > 767 ? false : true;
				if (mobileWidth && !created) {
					...
					created     = true;
					mobileWidth = true;
				} else if (!mobileWidth && created) {
					mCar.each(function(index){
						console.log(!created);
						$this = $(this);
						destroyCarousel($this, index);
					});
					created     = false;
					mobileWidth = false;
				}
			...
			function createCarousel(selector, index) {
				...
				window['JetimpexMobileCarousel_' + selector.data('mobile-carousel')] = window['JetimpexMobileCarousel_' + selector.data('mobile-carousel')] || [];
				...
				slider = new Swiper(selector, {
					...
					spaceBetween        : parseInt(settings.space)  || 30,
					paginationClickable : true,
					...
					breakpoints: {
						320: {
							slidesPerView: parseInt(settings.slides320)  || 1,
							spaceBetween: parseInt(settings.space320)    || 10
						},
						480: {
							slidesPerView: parseInt(settings.slides480)  || 1,
							spaceBetween: parseInt(settings.space320)    || 20
						},
						767: {
							slidesPerView: parseInt(settings.slides480)  || 1,
							spaceBetween: parseInt(settings.space320)    || 20
						...
						768: {
							slidesPerView: parseInt(settings.slides768)  || 2,
							spaceBetween: parseInt(settings.space320)    || 20
						},
						992: {
							slidesPerView: parseInt(settings.slides992)  || 2,
							spaceBetween: parseInt(settings.space320)    || 20

						},
						1200: {
							slidesPerView: parseInt(settings.slides1200) || 2,
							spaceBetween: parseInt(settings.space320)    || 20
						}
					}
				});
				window['JetimpexMobileCarousel_' + selector.data('mobile-carousel')][index] = {slider : slider, wrapperClass : wrapperClass, slideClass : slideClass};
			};
			function destroyCarousel(selector, index) {
				var slider = window['JetimpexMobileCarousel_' + selector.data('mobile-carousel')][index];
