...
(function ($) {
	$.fn.setMapTable = function () {
        ...
        $(document).on('click', function (e) {
        	var target = $(e.target);
        	if (!target.is('[data-mapid], .marker__cnt') && !target.parents('.marker__cnt').length > 0 || target.is('.marker__cnt .close-btn')) {
        ...
        $(document).on('click', '[data-mapid]', function (e) {
  ...
  function selectRowAndPart(id, select) {
    // select Row
    if (select) {
    	$("[data-mapid=" + id + "]:not(area)").addClass('active');
    }
    else {
    	$("[data-mapid=" + id + "]:not(area)").removeClass('active');
    }
    // Select Part
    $("area[data-mapid=" + id + "]").data('maphilight', { alwaysOn: select }).trigger('alwaysOn.maphilight'); // add/remove one
 }

 function displayPopup(id, select) {
 	if (select) {
 		$("[data-marker=" + id + "]").css({'z-index' : '10'}).addClass('active');
 		if ($(window).width() > 767) {
 			$("[data-marker=" + id + "] .marker__cnt").stop(true, false).css({display: 'block'}).animate({
 				opacity: 1,
 				'margin-top': '0',
 				'margin-bottom': '0'
 			}, 500);
 		...
 		$("[data-marker=" + id + "]").css({'z-index' : '5'}).removeClass('active');
 		if ($(window).width() > 767) {
 			$("[data-marker=" + id + "] .marker__cnt").stop(true, false).animate({
 				opacity: 0,
 				'margin-top': '30',
 				'margin-bottom': '30'
 			}, {
 				duration: 500,
 					...
 					$("[data-marker=" + id + "] .marker__cnt").css({display: 'none'});
 				}
 			});
 		}
 	}
