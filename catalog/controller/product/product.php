...
class Product extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		$specialDates = $this->model_catalog_category->getProductSpecialsDate( $this->request->get['product_id']);

		if(isset($specialDates['date_end']) && $specialDates['date_end'] != '0000-00-00'){
			$data['date_end']  = $specialDates['date_end'];
		}else{
			$data['date_end'] = '';
		}
		...
		$themeoption = $this->model_setting_extension->getThemeOption();
		$themeoption = $this->model_catalog_product->getThemeOption();
		$data['product_page_counter'] = (isset($themeoption['product_counter']) && $themeoption['product_counter'] == 1) ? $themeoption['product_counter'] : 0;	
		...
		if ($product_info) {
			...
			$data['stock_status'] = ($product_info['quantity'] <= 0) ? $this->language->get('text_out_of_stock') : $this->language->get('text_in_stock');
			$data['stock_statusclass'] = ($product_info['quantity'] <= 0) ? 'sold-out' : '';
			...
			foreach ($results as $result) {
				...
				$specialDates = $this->model_catalog_category->getProductSpecialsDate( $result['product_id']);
				...
				$product_data = [
					...
					'manufacturers' => $this->url->link('product/manufacturer|info', 'language=' . $this->config->get('config_language') . '&manufacturer_id=' .  $result['manufacturer_id'] . $url),
