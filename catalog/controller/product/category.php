...
class Category extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		if ($category_info) {
			...
			// print_r($data['product_counter']); exit;
			...
			foreach ($results as $result) {
				...
				if(!empty($result['image'])) {
					$subcategory_image = $this->model_tool_image->resize($result['image'], 370, 370);
				}else{
					$subcategory_image = $this->config->get('config_url') . 'image/no_image.png';
				}
			...
			foreach ($results as $result) {
				...
				$product_data = [
					...
					'thumb'       => $image,
					'name'        => $result['name'],
					'description' => substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, 150) . '..',
					'price'       => $price,
					'special'     => $special,
