...
class Cart extends \Opencart\System\Engine\Controller {
	...
	public function remove(): void {
		...
		if($this->cart->countProducts() > 1) {
				...
				$json['total'] = sprintf($this->language->get('text_item'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), '');
		}

		$json['text_items_small'] = $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0);
