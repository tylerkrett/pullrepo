...
class NewslettersubscribePopup extends \Opencart\System\Engine\Controller {
	private string $extensionPath = 'extension/newslettersubscribepopup/module/newslettersubscribepopup';

	private $error = array(); 
	
	public function index() {   
	
		$this->load->language('extension/newslettersubscribepopup/module/newslettersubscribepopup');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('newslettersubscribepopup', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/newslettersubscribepopup/module/newslettersubscribepopup', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title1'),
				'href' => $this->url->link('extension/newslettersubscribepopup/module/newslettersubscribepopup', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);			
		}
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/newslettersubscribepopup/module/newslettersubscribepopup', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/newslettersubscribepopup/module/newslettersubscribepopup', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->post['newsletter_banner'])) {
			$data['newsletter_banner'] = $this->request->post['newsletter_banner'];
		} elseif (!empty($module_info)) {
			
			$data['newsletter_banner'] = $module_info['newsletter_banner'];
		} else {
			$data['newsletter_banner'] = 'no_image.png';
		}

		if (isset($this->request->post['newsletter_banner']) && is_file(DIR_IMAGE . $this->request->post['newsletter_banner'])) {
			$data['imagethumb1'] = $this->model_tool_image->resize($this->request->post['newsletter_banner'], 100, 100);
		} elseif (!empty($module_info) && is_file(DIR_IMAGE . $module_info['newsletter_banner'])) {
			$data['imagethumb1'] = $this->model_tool_image->resize($module_info['newsletter_banner'], 100, 100);
		} else {
			$data['imagethumb1'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		$user_total = $this->model_extension_newslettersubscribepopup_ishithemes_newslettersubscribepopup->getTotalUsers();
		...
		$results = $this->model_extension_newslettersubscribepopup_ishithemes_newslettersubscribepopup->getList($filter);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('extension/newslettersubscribepopup/module/newslettersubscribepopup|unsubscribe', 'user_token=' . $this->session->data['user_token'] . '&user_id=' . $result['id'] . $url, true)
			);
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'url'   => $this->url->link('extension/newslettersubscribepopup/module/newslettersubscribepopup', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}')
		]);
		...
		$this->response->setOutput($this->load->view('extension/newslettersubscribepopup/module/newslettersubscribepopup', $data));
	}
	...
	public function unsubscribe() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach ($this->request->post['selected'] as $user_id) {
				$this->model_extension_newslettersubscribepopup_ishithemes_newslettersubscribepopup->delete($user_id);                
	  		}
			...
			$this->redirect($this->url->link('extension/newslettersubscribepopup/module/newslettersubscribepopup', 'user_token=' . $this->session->data['user_token'] . $url, true));
			
		} else if( isset($this->request->get['user_id']) ) {
			
			$data = $this->request->get['user_id'];
			
			$this->model_extension_newslettersubscribepopup_ishithemes_newslettersubscribepopup->delete($data);
			...
			$this->response->redirect($this->url->link('extension/newslettersubscribepopup/module/newslettersubscribepopup', 'user_token=' . $this->session->data['user_token'] . $url, true));	
	...
	protected function validateDelete() {
    	if (!$this->user->hasPermission('modify', 'extension/newslettersubscribepopup/module/newslettersubscribepopup')) {
	...
	private function validate() {
		...
		if (!$json) {
			...
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('newslettersubscribepopup.newslettersubscribepopup', $this->request->post);
			} else {
				$this->model_setting_module->editModule($this->request->get['module_id'], $this->request->post);
			}
