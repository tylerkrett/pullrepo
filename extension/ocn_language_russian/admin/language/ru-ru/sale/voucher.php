<?php
// Heading
$_['heading_title']     = 'Подарочные сертификаты';

// Text
$_['text_success']      = 'Успех: Вы изменили сертификаты!';
$_['text_list']         = 'Список подарочных сертификатов';
$_['text_add']          = 'Добавление подарочного сертификата';
$_['text_edit']         = 'Редактирование подарочного сертификата';
$_['text_sent']         = 'Успех: Подарочный сертификат успешно отправлен на указанный e-mail адрес!';
$_['text_history']      = 'История';

// Column
$_['column_name']       = 'Название сертификата';
$_['column_code']       = 'Код';
$_['column_from']       = 'От';
$_['column_to']         = 'Кому';
$_['column_theme']      = 'Тема';
$_['column_amount']     = 'Сумма';
$_['column_status']     = 'Статус';
$_['column_order_id']   = 'Номер заказа';
$_['column_customer']   = 'Покупатель';
$_['column_date_added'] = 'Дата добавления';
$_['column_action']     = 'Действие';

// Entry
$_['entry_code']        = 'Код';
$_['entry_from_name']   = 'Имя отправителя';
$_['entry_from_email']  = 'Эл. почта отправителя';
$_['entry_to_name']     = 'Имя получателя';
$_['entry_to_email']    = 'Эл. почта получателя';
$_['entry_theme']       = 'Тема';
$_['entry_message']     = 'Сообщение';
$_['entry_amount']      = 'Сумма';
$_['entry_status']      = 'Статус';

// Help
$_['help_code']         = 'Код, который клиент вводит для активации сертификата.';

// Error
$_['error_permission']  = 'Внимание: У вас нет прав на изменение купонов!';
$_['error_exists']      = 'Внимание: Код купона уже используется!';
$_['error_code']        = 'Код должен быть от 3 до 10 знаков!';
$_['error_to_name']     = 'Название получателя должно быть от 1 до 64 символов!';
$_['error_from_name']   = 'Ваше имя должно быть от 1 до 64 символов!';
$_['error_email']       = 'Адрес электронной почты недействителен!';
$_['error_amount']      = 'Сумма должна быть больше или равна 1!';
$_['error_order']       = 'Внимание: Этот купон не может быть удален, поскольку он является частью <a href="%s">заказа </a>!';