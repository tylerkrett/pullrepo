...
class Blogger extends \Opencart\System\Engine\Controller {   
	...
	public function index() {
		...
		// print_r($this->config->get('config_language')); die;
		
		if (!isset($this->request->get['blogger_id'])) {
			$this->response->redirect($this->url->link('extension/blogger/information/blogger|blogs', 'language=' . $this->config->get('config_language'), true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/blogger/information/blogger', 'language=' . $this->config->get('config_language'), '', true)
		);

		$data['breadcrumbs'][] = array(
			'text' => $blog_info['title'],
			'href' => $this->url->link('extension/blogger/information/blogger', 'language=' . $this->config->get('config_language') . '&blogger_id=' . $this->request->get['blogger_id'], true)
		);
		...
		$data['action'] = $this->url->link('extension/blogger/information/blogger', 'language=' . $this->config->get('config_language'). '&blogger_id=' . $this->request->get['blogger_id'] , true);
	...
	private function validate() {
		if ((oc_strlen(trim($this->request->post['author'])) < 1) || (oc_strlen(trim($this->request->post['author'])) > 32)) {
		...
		if ((oc_strlen($this->request->post['comment']) < 10) || (oc_strlen($this->request->post['comment']) > 3000)) {
			...
			if (!empty($setting['status']) && $setting['status']) {
				...
				$total_comments = $this->model_extension_blogger_module_blogger->getTotalBlogComments($module_blog['blogger_id']);
					...
					$module_blog_data[] = array(
						...
						'href'        => $this->url->link('extension/blogger/information/blogger', 'blogger_id=' . $module_blog['blogger_id'] .'&language=' . $this->config->get('config_language'), true)
					);
				}
